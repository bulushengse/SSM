...
function sendEm(){
	...
	$.ajax({
		type: "POST",
		url: locat+'/tool/sendEmail.do?tm='+new Date().getTime(),
    	data: {EMAIL:EMAIL,TYPE:TYPE,TITLE:TITLE,CONTENT:CONTENT,isAll:isAll},
		dataType:'json',
		...
		success: function(data){
			 $.each(data.list, function(i, list){
				 if(list.msg == 'ok'){
					 ...
					 $("#msg").tips({
						side:3,
			            msg:'成功发出'+count+'条,失败'+ecount+'条,如有失败检查邮箱地址格式!',
			            bg:'#68B500',
			            time:5
				      });
				 }else{
